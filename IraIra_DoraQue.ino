...
void Jokyoku() { //ドラクエ序曲
  const int JokyokuNum = 69; const int QN = 450; const int oct = 6;
  int JokyokuDeta[JokyokuNum][2] = {
    {ET[4][oct - 1], QN * 2 / 3}, {ET[4][oct - 1], QN / 4}, {ET[0][oct], QN}, {ET[1][oct], QN}, {ET[2][oct], QN}, {ET[3][oct], QN}, {ET[4][oct], QN},
    {ET[0][oct + 1], QN * 2},
    {ET[6][oct], QN * 2 / 3}, {ET[5][oct], QN / 4}, {ET[5][oct], QN * 3 / 2}, {ET[4][oct], QN / 2}, {0, QN / 2},
    {ET[9][oct], QN / 2}, {ET[9][oct], QN / 2}, {ET[5][oct], QN / 2}, {ET[4][oct], QN}, {ET[2][oct], QN * 3 / 2}, {0, QN / 2},
    {ET[2][oct - 1], QN * 2 / 3}, {ET[2][oct - 1], QN / 4}, {ET[2][oct - 1], QN}, {ET[2][oct - 1], QN},
    {ET[9][oct - 1], QN}, {ET[10][oct - 1], QN}, {ET[5][oct - 1], QN * 2}, {0, QN / 2},
    {ET[5][oct - 1], QN / 2}, {ET[6][oct - 1], QN / 2}, {ET[0][oct], QN / 2}, {ET[1][oct], QN * 2}, {0, QN / 2},
    {ET[5][oct - 1], QN / 2}, {ET[5][oct - 1], QN / 2}, {ET[0][oct], QN / 2}, {ET[0][oct], QN},

    {ET[6][oct - 1], QN}, {ET[5][oct - 1], QN}, {ET[4][oct - 1], QN},
    {ET[2][oct], QN * 2}, {0, QN / 4},

    {ET[3][oct], QN / 2}, {ET[2][oct], QN / 2}, {ET[1][oct], QN / 2}, {ET[0][oct], QN * 2}, {ET[5][oct - 1], QN},
    {ET[0][oct], QN},

    {ET[1][oct], QN * 2}, {0, QN / 2},
    {ET[2][oct], QN / 2}, {ET[1][oct], QN / 2}, {ET[0][oct], QN / 2}, {ET[0][oct], QN * 2},
    {ET[6][oct - 1], QN}, {0, QN / 8}, {ET[4][oct - 1], QN}, {ET[4][oct], QN * 2}, {0, QN / 4},
    {ET[2][oct], QN / 2}, {ET[3][oct], QN / 2}, {ET[4][oct], QN / 2}, {ET[5][oct], QN * 2}, {0, QN / 2},
    {ET[5][oct - 1], QN / 2}, {ET[6][oct - 1], QN / 2}, {ET[0][oct], QN / 2}, {ET[3][oct], QN * 2},
    {ET[2][oct], QN * 2}, {ET[0][oct], QN * 2}
  };
  while (StartFlg == true && MistakeFlg == false && GoalFlg == false) {
    TryingBreakTime(100);
    for (int i = 0; i < JokyokuNum; i++) { // メロディーを流す
      tone(buzzerPin, JokyokuDeta[i][0], JokyokuDeta[i][1]);
      if (HitPoint == 1 || millis() - StartTime > EndTime - 10000) {
        TryingBreakTime(JokyokuDeta[i][1] / 1.5);
      } else {
        TryingBreakTime(JokyokuDeta[i][1]);
      }
